...
#include <XGRealTimeSpeechToTextResponseType.h>
...
void UXGKeDaXunFeiSocketSubsystem::XGBeginSpeachToTextConfig(
FXGRealTimeSpeechToTextRequestType InXGRealTimeSpeechToTextType, 
FXGBeginSpeachToTextDelegate InXGBeginSpeachToTextDelegate, 
FXGSpeachToTextDelegate InXGSpeachToTextDelegat,
FXGSpeachToTextWithoutTranslateDelegate InXGSpeachToTextWithoutTranslateDelegate
)
{
	if (bSpeechToText)
	{
		InXGBeginSpeachToTextDelegate.ExecuteIfBound(false);
	...
	XGBeginSpeachToTextDelegate = InXGBeginSpeachToTextDelegate;
	XGSpeachToTextDelegate = InXGSpeachToTextDelegat;
	XGSpeachToTextWithoutTranslateDelegate = InXGSpeachToTextWithoutTranslateDelegate;
	...
	ServerURL += InXGRealTimeSpeechToTextType.GenerateRequireParams();
	...
	Socket->OnMessage().AddUObject(this, &UXGKeDaXunFeiSocketSubsystem::OnMessageFullConfig);
...
void UXGKeDaXunFeiSocketSubsystem::CloseSocket()
{
	...
	XGBeginSpeachToTextDelegate.Clear();
	XGSpeachToTextDelegate.Clear();
	XGSpeachToTextWithoutTranslateDelegate.Clear();
...
void UXGKeDaXunFeiSocketSubsystem::OnMessageFullConfig(const FString& Message)
{
	UE_LOG(LogTemp, Warning, TEXT("%s Message:%s"), *FString(__FUNCTION__), *Message);
	...
	if (!Message.IsEmpty())
	{
		...
		if (ResultObj->TryGetStringField("action", MessageAction))
		{
			...
			if (MessageAction.Equals(TEXT("result")))
			{
				...
				//Message
				FXGRealTimeSpeechToTextResponseType XGRealTimeSpeechToTextResponseType;
				FJsonObjectConverter::JsonObjectStringToUStruct(Message, &XGRealTimeSpeechToTextResponseType);


				TSharedPtr<FJsonObject> ResultDataObj;
				TSharedRef<TJsonReader<>> DataReader = TJsonReaderFactory<>::Create(XGRealTimeSpeechToTextResponseType.data);
				...
				{
					//处理带有实时翻译的数据
					FXGRealTimeSpeechToTextResponseTranslateData XGRealTimeSpeechToTextResponseTranslateData;
					if (FJsonObjectConverter::JsonObjectStringToUStruct(XGRealTimeSpeechToTextResponseType.data, &XGRealTimeSpeechToTextResponseTranslateData))
					{
						if (XGRealTimeSpeechToTextResponseTranslateData.ed > 0)
						{
							FString dst = XGRealTimeSpeechToTextResponseTranslateData.dst;
							FString src = XGRealTimeSpeechToTextResponseTranslateData.src;
				...
				{
					//处理不带有实时翻译的数据
					FXGRealTimeSpeechToTextResponseNoTranslateData XGRealTimeSpeechToTextResponseNoTranslateData;
					if (FJsonObjectConverter::JsonObjectStringToUStruct(XGRealTimeSpeechToTextResponseType.data, &XGRealTimeSpeechToTextResponseNoTranslateData))
					{
						//判断是中间结果还是 最终结果
						//bool bFinal = XGRealTimeSpeechToTextResponseNoTranslateData.cn.st.type.Equals(TEXT("1"));
						//判断句子是否句子结束
						bool bFinal = !XGRealTimeSpeechToTextResponseNoTranslateData.cn.st.ed.Equals(TEXT("0"));

						if (bFinal)
						{
							TArray<FXGRealTimeSpeechToTextResponseNoTranslateWSData> WSDatas = XGRealTimeSpeechToTextResponseNoTranslateData.cn.st.rt[0].ws;
							...
							UE_LOG(LogTemp, Error, TEXT("SST :%s "), *OutText);
							XGSpeachToTextWithoutTranslateDelegate.ExecuteIfBound(OutText);
						}
